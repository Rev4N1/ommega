...
pub struct Config {
    ...
    /// Aggregated-payment (Yipay/Mapay) gateway URL, e.g. https://pay.example.com/submit.php
    pub pay_gateway: String,
    /// Aggregated-payment merchant id (pid)
    pub pay_pid: String,
    /// Aggregated-payment merchant key, used for signing
    pub pay_key: String,
    /// Aggregated-payment async notify URL (notify_url)
    pub pay_notify_url: String,
    /// Aggregated-payment sync return URL (return_url)
    pub pay_return_url: String,
    /// Product name prefix, e.g. "OMMEGA activation card"
    pub pay_product_name: String,
    /// Optional fallback verification string used after callback signature passes (backward-compatible with CARD_CALLBACK_SECRET)
    pub pay_callback_secret: String,
    /// Whether authentication is required (default true). When false, anonymous access is allowed if no token is configured.
    pub auth_required: bool,
    /// Pending task TTL (seconds). Pending tasks not claimed within this time are marked failed.
    pub pending_ttl_secs: u64,
    /// Max number of completed/failed tasks to retain (counted independently per category).
    pub completed_max: usize,
    /// Retention time for completed/failed tasks (seconds).
