...
#[test]
fn no_carrier_ommega_key_entry_reply_uses_synthetic_security_level_mapping() {
    ensure_binder_process_state();
    ...
    let mut reply = build_no_carrier_ommega_key_entry_reply(
        KeyEntryResponse {
    ...
    .expect("no-carrier ommega key-entry reply should serialize");
...
fn synthetic_operation_carrier_forwards_update() {
    ...
    .expect("synthetic update should return an ommega-owned reply");
    let (data, data_size, offsets, offsets_size) = raw_parts(&mut reply);
...
fn synthetic_operation_abort_keeps_tombstone_returning_invalid_handle() {
    ...
    .expect("synthetic abort should return an ommega-owned reply");
    let (data, data_size, offsets, offsets_size) = raw_parts(&mut reply);
...
fn raw_create_operation_reply_still_does_not_register_operation_mapping() {
    ...
    let ommega_operation = BnKeystoreOperation::new_binder(TestOperationBackend {
        update_output: vec![9, 9, 9],
        aborts: Arc::new(AtomicUsize::new(0)),
        update_aad_status: None,
    });
    let mut reply = parcel::build_create_operation_reply(CreateOperationResponse {
        r#iOperation: Some(ommega_operation),
        r#operationChallenge: None,
        r#parameters: None,
        r#upgradedBlob: Some(vec![7, 7]),
    })
    .expect("direct ommega createOperation reply should serialize");
    let (data, data_size, offsets, offsets_size) = raw_parts(&mut reply);
    let parsed: CreateOperationResponse =
        unsafe { parcel::parse_success_reply(data, data_size, offsets, offsets_size) }
            .expect("direct ommega createOperation reply should parse");
    ...
    assert!(
        OPERATION_TARGETS
            ...
            .expect("operation target map poisoned")
            .is_empty(),
        "direct ommega replies should not register a fake system carrier mapping"
    );
}
