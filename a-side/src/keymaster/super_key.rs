...
impl SuperKeyManager {
    ...
    pub fn unwrap_key_if_required_with_ommega_compatibility<'a>(
        &self,
        metadata: &BlobMetaData,
        blob: &'a [u8],
    ) -> Result<KeyBlob<'a>> {
        ...
        match self.try_unwrap_key_with_ommega_legacy(metadata, blob) {
            Ok(Some(key_blob)) => Ok(key_blob),
            Ok(None) | Err(_) => Err(aosp_error),
        }
    }

    fn try_unwrap_key_with_ommega_legacy<'a>(
        &self,
        metadata: &BlobMetaData,
        blob: &'a [u8],
        ...
        let key = match key_id {
                ...
                if super_key.algorithm == SuperEncryptionAlgorithm::EcdhP521 =>
            {
                ...
                match ECDHPrivateKey::from_private_key(&super_key.key).and_then(|key| {
                    key.decrypt_message_ommega_legacy(public_key, salt, iv, blob, aead_tag)
                }) {
                    Ok(key) => key,
                    Err(_) => return Ok(None),
                }
    ...
    fn populate_cache_from_super_key_blob(
        ...
        let super_key = Self::extract_super_key_from_key_entry_with_ommega_compatibility(
    ...
    pub(crate) fn extract_super_key_from_key_entry_with_ommega_compatibility(
        algorithm: SuperEncryptionAlgorithm,
        entry: KeyEntry,
        pw: &Password,
        reencrypt_with: Option<Arc<SuperKey>>,
        ...
        let key = match pw
            .derive_key_ommega_legacy(&salt, AES_256_KEY_LENGTH)
    ...
    fn get_or_create_super_key(
        ...
        if let Some((_, key_entry)) = loaded_key {
            Ok(
                Self::extract_super_key_from_key_entry_with_ommega_compatibility(
                    key_type.algorithm,
                    key_entry,
                    password,
                    reencrypt_with,
    ...
    /// Unlocks an existing user, or initializes the user's super keys if ommega missed the
...
mod test {
    ...
    fn test_legacy_boot_level_blob_is_reencrypted() -> Result<()> {
        ...
        *manager.data.legacy_boot_level_key_cache.get_mut().unwrap() = Some(
        ...
        let unwrapped =
            manager.unwrap_key_if_required_with_ommega_compatibility(&metadata, &ciphertext)?;
