...
use kmr_crypto_ommega::{ec::OmmegaEc, hmac::OmmegaHmac, rng::OmmegaRng};
use kmr_ta::device::{
    ...
    fn generate_dice_artifacts(
        ...
        let key_material = match self.sign_algo {
            CsrSigningAlgorithm::EdDSA => {
                let secret = self.derive_bytes_from_hbk(&OmmegaHmac, b"Device Key Seed", 32)?;
                ec::import_raw_ed25519_key(&secret)
            }
            // TODO: generate the *same* key after reboot, by use of the TPM.
            CsrSigningAlgorithm::ES256 => {
                ec.generate_nist_key(&mut OmmegaRng, ec::NistCurve::P256, &[])
            }
            CsrSigningAlgorithm::ES384 => {
                ec.generate_nist_key(&mut OmmegaRng, ec::NistCurve::P384, &[])
            }
        }?;
